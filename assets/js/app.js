...
// Initializes scrolly.js for /scrolly route only with cleanup. Boots immediately. Works on GitHub Pages subpaths.
...
const pageMeta = {
  ...
  '/scrolly':      { title: 'scrolly teller — 1807-Chain',       desc: 'Story & showcase' },
  ...
  '/construction': { title: 'Construction — 1807-Chain',  desc: 'Window/door install, siding, painting' },
...
async function boot() {
  let scrollyCleanup = null;

  try {
    // Delay to allow landing.js to initialize
    ...
    // Initialize scrolly.js for /scrolly route
    const rk = routeKey();
    if (rk === '/scrolly') {
      console.log('Initializing scrolly.js for /scrolly route');
      try {
        scrollyCleanup = initScrolly();
      } catch (err) {
        console.error('Failed to initialize scrolly.js:', err);
      }
    } else if (rk === '/') {
      // Skip focus for landing page to avoid animation disruption
    ...
    // Handle navigation cleanup for SPA-like transitions
    const originalPushState = history.pushState;
    history.pushState = function (...args) {
      const newRoute = routeKey(window.location.pathname);
      if (scrollyCleanup && newRoute !== '/scrolly') {
        console.log('Cleaning up scrolly.js on navigation');
        try {
          scrollyCleanup.cleanup();
        } catch (err) {
          console.error('Scrolly cleanup failed:', err);
        }
        scrollyCleanup = null;
      }
    ...
    // Cleanup on page unload (scoped to scrolly.js)
    window.addEventListener('beforeunload', () => {
      if (scrollyCleanup) {
        try {
          scrollyCleanup.cleanup();
        } catch (err) {
          console.error('Scrolly cleanup on unload failed:', err);
        }
      }
    });
  } catch (err) {
