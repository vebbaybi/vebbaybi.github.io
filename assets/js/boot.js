/* =========================================================
   ...
   - Gates ONLY on /home when:
       • You arrive FROM a landing page: '/', '/scrolly'
...
(function () {
  ...
  const LANDING_KEYS = new Set(['/', '/scrolly']);
  ...
  ready(function () {
    ...
    /*Optional session guard: once per tab unless forced
    ...
    if (!shouldGate) {
      siteReady({
        bypass: true,
        reason: bypass ? 'dev_bypass' : atHome ? (alreadyGated ? 'session_already_gated' : 'not_from_landing') : 'non_home'
      });
      return;
    }
